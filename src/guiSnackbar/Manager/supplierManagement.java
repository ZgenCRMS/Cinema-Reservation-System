...
//       super(parent, view);""
    ...
    private void hint() {
        if (jTextField6 != null) {
            jTextField6.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "Search Name");
        }
    }
    ...
    private void loardSuppliers(String col, String orderby, String fname) {
        try {

            ResultSet resultset = mySQL.executeSearch("SELECT * FROM `snack_supplier` INNER JOIN `company` ON `snack_supplier`.`company_id`=`company`.`id` WHERE `fname` LIKE '" + fname + "%' ORDER BY `" + col + "` " + orderby + "");
...
//                this.repaint();
    ...
    private void jTextField3KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTextField3KeyReleased
       String tx = jTextField3.getText();

        if (!tx.matches("\\d*")) {

            jTextField3.setText(tx.replaceAll("[^\\\\d]", ""));
